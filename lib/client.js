 ...
 * 面板高度随消息条数自适应（240–520px 封顶）并上下居中。
 * 会话历史是分页窗口：导航只列出已加载的提问，标题显示
 * 「已加载 / 全部」（全部 = 整份日志的 sessionStats.turns）；
 * 还有更早未加载时，列表底部出现「加载更早消息」按钮直接翻页。
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined'
      ...
      tag.textContent = [
        '.dsh-nav-zone{position:fixed;pointer-events:auto;display:flex;align-items:center;',
        ...
        '.dsh-nav-list{overflow-y:auto;padding:0 8px 8px;display:flex;flex-direction:column;gap:1px;flex:1;',
        'min-height:0;}',
        ...
        '.dsh-nav-more-btn:hover:not(:disabled){color:var(--dsw-alias-label-primary);',
    ...
    const plugin = {
      ...
      apply(ctx) {
        ...
        // ---- 会话快照 + 整份日志轮次总数（sessionStats 投影）的可观察源 ----
        ...
        const refresh = () => {
          ...
          cached = { session, total: wholeLogTurns(source) }
        ...
        // ---- 面板组件：悬停滑出（ChatGPT 式），分页完善 ----
        function ChatNavPanel(props) {
          const state = props.useChatNav(s => s)
          ...
          // 收起态：触发区域 = 手柄本身（48×96，垂直居中）；横杆数 = 已加载消息条数
          if (!open) {
            ...
            return React.createElement('div', {
              ...
              style: {
                right: geom.right,
                top: geom.topBase + Math.max(0, (geom.areaH - HANDLE_H) / 2),
                height: HANDLE_H,
                width: HANDLE_W,
              },
          ...
          // 计数口径统一（修复 31/30 类矛盾）：
          //  - total = sessionStats.turns（已关闭轮次）；若存在尚未计入的运行中轮次（open 且无 closed step），补 1
          //  - 展示规则：部分加载时 A / B；全部加载完（A == B）显示 A；A > B（口径残留差异）只显示 A
          let effectiveTotal = total
          if (effectiveTotal !== undefined && session !== undefined && session !== null) {
            let openHasClosedStep = false
            let hasOpenTurn = false
            const turns = session.chat.timeline.turns
            for (const t of turns.values()) {
              if (t.status !== 'open') continue
              hasOpenTurn = true
              if (t.steps.some(step => step.status === 'closed')) openHasClosedStep = true
              break
            }
            if (hasOpenTurn && !openHasClosedStep) effectiveTotal += 1
          }
          const countText = effectiveTotal === undefined || items.length > effectiveTotal
            ? String(items.length)
            : items.length === effectiveTotal
              ? String(items.length)
              : String(items.length) + ' / ' + effectiveTotal
          ...
          return React.createElement('div', {
            ...
            React.createElement('div', { className: 'dsh-nav-head' },
              ...
              React.createElement('span', { className: 'dsh-nav-count' }, countText),
            ),
