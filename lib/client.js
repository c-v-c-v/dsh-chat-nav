    ...
    const plugin = {
      ...
      apply(ctx) {
        ...
        function ChatNavPanel(props) {
          ...
          // 手柄与面板位置。该滚动容器在会话页常驻，但需同时确认当前是
          // 「对话」视图——只有对话视图内才有 [data-chat-flow]（轨迹等其它
          // 视图没有），借此排除切换到轨迹 tab 时手柄仍显示的问题。
          // details 列展开时不显示手柄（避开其拖拽手柄，用帧根
          // data-details-collapsed 检测）。
          React.useEffect(() => {
            const measure = () => {
              const scrollport = document.querySelector('[data-conversation-scroll]')
              // 仅当滚动容器存在且其内渲染了对话流（当前为对话视图）时定位。
              if (scrollport === null || scrollport.querySelector('[data-chat-flow]') === null) {
                setGeometry(null)
                return
              }
            ...
            // 监听滚动容器子树变化（对话/轨迹视图切换时 [data-chat-flow]
            // 挂载或卸载），确保切 tab 后立即重测，及时隐藏/显示手柄。
