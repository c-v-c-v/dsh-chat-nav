...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 流程控制类节点（命令/压缩）所在轮次：不算"提问"，不进导航列表，计数分母需剔除。 */
    const COMMAND_TURN_KINDS = new Set(['command', 'command-input', 'manual-compaction', 'compaction'])

    /**
     * 由会话快照构建导航：一条 = 一个用户提问（user/steering）。
     * 命令/压缩类流程轮次不生成导航项（避免「命令」假项）；同时返回
     * "消息行 → 导航项"映射供滚动高亮，以及命令类轮次集合，供计数
     * 分母在窗口=全日志时剔除（流程控制不占提问名额）。
     */
    function buildNavItems(session) {
      const chat = session.chat
      const items = []
      const rowToItem = new Map()
      const commandTurns = new Set()
      for (const key of chat.order) {
        ...
        } else if (turn !== undefined && COMMAND_TURN_KINDS.has(kind)) {
          commandTurns.add(turn)
        }
        const last = items[items.length - 1]
        if (last !== undefined) rowToItem.set(key, last.key)
      }
      return { items, rowToItem, commandTurns }
    }
     ...
     * 宿主权威）。注意口径：该投影对任何出现过 step/end 的轮次计数——
     * 包括仍在运行、但已至少结束一个 step 的轮次。
     */
    ...
    /**
     * 计数展示文案。窗口未加载完（hasMore === true）时不显示分母——
     * 分母来自 sessionStats.turns（全日志口径），此时混着未加载历史，
     * 直接显示会让「2/3 中 3 比已加载提问多」的错觉重现；改为「已加载 N」
     * 并配合「加载更早消息」按钮提示未完成。
     * 窗口 = 全日志（hasMore === false）时窗口内 step 状态完整可靠，
     * 分母修正才成立：
     *   1) 剔除已被 turns 计入的命令类轮次（流程控制不占提问名额）；
     *   2) 补正在运行、且尚未产生任何 step/end 的末尾轮次（原补偿意图，
     *      限定在全量窗口下执行，避免部分加载时把运行轮次重复 +1）。
     * 展示规则：已加载 == 分母 → 单数；残留差异 → 单数（避免「6/5」）。
     */
    function navCountLabel(itemCount, total, session, commandTurns) {
      ...
      if (effective !== undefined) {
        ...
        // 1) 剔除已被 turns 计入的命令类轮次（该轮次至少一个 closed step）
        if (commandTurns !== undefined) {
          for (const turnNumber of commandTurns) {
            const turn = timeline.turns.get(turnNumber)
            if (turn !== undefined && turn.steps.some(step => step.status === 'closed')) {
              effective -= 1
            }
          }
        }
        // 2) 补正在运行、尚无任何 step/end 的末尾轮次
        if (session.running === true) {
          const lastTurnNumber = timeline.turnOrder[timeline.turnOrder.length - 1]
          if (lastTurnNumber !== undefined) {
            const turn = timeline.turns.get(lastTurnNumber)
            if (turn !== undefined && turn.status === 'open' && !turn.steps.some(step => step.status === 'closed')) {
              effective += 1
            }
          }
        }
    ...
    const EMPTY_NAV = { items: [], rowToItem: new Map(), commandTurns: new Set() }
          ...
          return React.createElement('div', {
            ...
            React.createElement('div', { className: 'dsh-nav-head' },
              ...
              React.createElement('span', { className: 'dsh-nav-count' },
                navCountLabel(items.length, total, session, nav.commandTurns),
              ),
            ),
